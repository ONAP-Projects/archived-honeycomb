...
final class X0005b9LteCrudService implements CrudService<X0005b9Lte> {
    ...
    public void writeData(@Nonnull final InstanceIdentifier<X0005b9Lte> identifier
	    ...
	    LOG.info("AMOL: - ConfigJsonHandler.getConfigJsonHandler(null).isConfigTopoInitialized value {}", ConfigJsonHandler.getConfigJsonHandler(null).isConfigTopoInitialized);

            if(!ConfigJsonHandler.getConfigJsonHandler(null).isConfigTopoInitialized) {
                LOG.info("AMOL: ConfigTopo not Initialized, so do nothing");
	    ...
	    int stIndex  = identifier.toString().indexOf("[_alias=");
            ...
            String cid = "";
            if(stIndex == -1) {
            	stIndex  = identifier.toString().indexOf("{_alias=");
            	subStr = identifier.toString().substring(stIndex+"{_alias=".length());
                cid = subStr.substring(0, subStr.indexOf("}"));
            } else {
                subStr = identifier.toString().substring(stIndex+"[_alias=".length());
                cid = subStr.substring(0, subStr.indexOf("]"));
            }

	    LOG.info("AMOL: subStr = {}",subStr);
            LOG.info("AMOL: cid = {}",cid);
	    ...
	    LOG.info("AMOL: modifyPci = {}",modifyPci);
            LOG.info("AMOL: modifyPci.isAllSet() = {} ",modifyPci.isAllSet());

            if(modifyPci.isAllSet()) {
                String jsonStr = new Gson().toJson(modifyPci, ModifyPci.class);
		LOG.info("AMOL: jsonStr = {} ",jsonStr);
		     ...
		     LOG.info("AMOL: else block called");
    ...
    public void updateData(@Nonnull final InstanceIdentifier<X0005b9Lte> identifier, @Nonnull final X0005b9Lte dataOld,
	    ...
	    LOG.info("AMOL: - ConfigJsonHandler.getConfigJsonHandler(null).isConfigTopoInitialized value {}", ConfigJsonHandler.getConfigJsonHandler(null).isConfigTopoInitialized);
            if(!ConfigJsonHandler.getConfigJsonHandler(null).isConfigTopoInitialized) {
		LOG.info("AMOL: CRITICAL ConfigTopo not initialized, Returning");
		ConfigJsonHandler.getConfigJsonHandler(null).isConfigTopoInitialized = true;
                //return;
	    }
            int stIndex  = identifier.toString().indexOf("[_alias=");
